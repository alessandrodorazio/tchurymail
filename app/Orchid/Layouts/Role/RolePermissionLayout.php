<?php
    ...
    class RolePermissionLayout extends Rows {
        ...
        private function generatedPermissionFields (Collection $permissionsRaw): array {
            return $permissionsRaw->map (function (Collection $permissions, $title) {
        ...
        private function makeCheckBoxGroup (Collection $permissions, string $title): Collection {
            return $permissions->map (function (array $chunks) {
                return $this->makeCheckBox (collect ($chunks));
            })->flatten ()->map (function (CheckBox $checkbox, $key) use ($title) {
                return $key === 0 ? $checkbox->title ($title) : $checkbox;
            })->chunk (4)->map (function (Collection $checkboxes) {
                return Group::make ($checkboxes->toArray ())->alignEnd ()->autoWidth ();
            });
        }
